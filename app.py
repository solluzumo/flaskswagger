...
def increase_price(service_id):
    """
    Повышение цены медицинской услуги по ее идентификатору.
    ---
    tags:
      - services
    parameters:
      - name: service_id
        in: path
        description: Идентификатор медицинской услуги.
        required: true
        type: integer
      - name: price
        in: formData
    ...
    responses:
      200:
        description: Успешное повышение цены медицинской услуги.
      404:
        description: Медицинская услуга не найдена.
    """
    service = MedicalService.query.get(service_id)
...
def get_max_duration():
    """
    Получение максимальной продолжительности врачебных услуг и их названия.
    ---
    tags:
      - services
    responses:
      200:
        description: Успешное получение максимальной продолжительности и названия услуги.
    """
    max_duration = db.session.query(func.max(MedicalService.duration)).scalar()
    service = db.session.query(MedicalService).filter_by(duration=max_duration).first()

    if service:
        return jsonify({'max_duration': max_duration, 'service_name': service.name})
    else:
